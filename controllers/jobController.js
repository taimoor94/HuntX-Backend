...
export const applyForJob = async (req, res) => {
  const { jobId, fullName, email, phone, linkedin, workExperience, education, skills, question1 } =
    req.body;
  const { coverLetter, resume } = req.files;
  ...
  try {
    ...
    const companies = jobs.reduce((acc, job) => {
      ...
      if (!acc[companyName]) {
        acc[companyName] = {
          name: companyName,
          logo: "https://via.placeholder.com/100", // Placeholder; in a real app, fetch from company profile
          jobCount: 0,
        };
      }
